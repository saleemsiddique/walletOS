...
**Progreso (2026-07-05):** 🚧 implementación en marcha en Mac (Xcode 26.6, Swift 6.3). **Ramas 1–13 completas en `develop`** (PRs #146, #148–#152, #154–#156, #158, #164, #165) — **Bloques A, B y C (autenticación completa) cerrados.** Proyecto generado con **XcodeGen**, arquitectura **feature-first** (`Features/`+`Core/`+`Shared/`), capa de red (interceptor Bearer + refresh coalesced), Keychain/TokenStore, base de datos local GRDB, motor de sincronización offline-first (cola FIFO + backoff), configuración de entornos, y autenticación completa: email+password, Sign in with Apple, Google Sign In, forgot/reset password con deep link `walletos://reset`. Verificación en simulador iPhone 17 (iOS 26.5) como criterio de "hecho" (Apple/Google e2e reales pendientes de dispositivo/cuenta). Siguiente: Bloque D — setup inicial y **Home**. Plan detallado por rama en [`docs/phase-10-ios-app.md`](docs/phase-10-ios-app.md).
